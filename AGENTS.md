...
| `backend/suggested_questions.py` | Personalized new-chat suggestions: chat history + optional query KB retrieval + OpenAI (`GET /conversations/suggested-questions`) |
...
| `backend/query_kb_store.py` + `backend/agents/query_kb.py` | Optional **query knowledge base** (pgvector + `text-embedding-3-small`): match before planner, interrupt for Re-run vs Adapt; see `supabase_migrations/migrations/003_query_kb.sql` |
| `backend/core/retry.py` | Shared exponential backoff for Supabase + LLM invokes |
...
- **Suggested questions (`/conversations/suggested-questions`)** – Uses `get_user_frequent_questions` (migration **`005_user_frequent_questions.sql`**) and `get_user_recent_questions` (**`006_user_recent_questions.sql`**). If the recent RPC is missing, the backend falls back without recent context. Env toggles (hackathon quota): **`SUGGESTED_QUESTIONS_ENABLED`** (default `1`; set `0` to return empty suggestions), **`SUGGESTED_QUESTIONS_LLM`** (default `1`; set `0` to skip all model calls for this feature — no embeddings for KB match and no suggestion model; falls back to history-only or generic list), **`SUGGESTED_QUESTIONS_INCLUDE_KB`** (default `1`; set `0` or pass `?include_kb=false` to skip query-KB retrieval + embedding call when LLM is enabled), **`SUGGESTED_QUESTIONS_CACHE_TTL_SEC`** (default `600`; set `0` to disable in-process cache per user).
- **Windows + OneDrive + `frontend/.next`** – If `next dev` logs **`EBUSY: resource busy or locked`** on files under **`frontend/.next`** (e.g. `server/webpack-runtime.js`), the dev server cannot read its build output and the browser may show **Internal Server Error**. Stop all Node/dev processes, delete **`frontend/.next`**, then restart `npm run dev`. Longer-term: pause OneDrive while developing, clone the repo **outside** OneDrive (e.g. `C:\dev\...`), or exclude **`frontend/.next`** from sync if your client supports it.
